...
class IMeetingCollectionTile(IPersistentCoverTile):
    ...
    form.omitted('start')
    form.no_omit(IDefaultConfigureForm, 'start')
    date = schema.Datetime(
        title=_(u'Start'),
        required=False,
    )
    ...
    form.omitted('end')
    form.no_omit(IDefaultConfigureForm, 'end')
    date = schema.Datetime(
        title=_(u'End'),
        required=False,
    )
...
class MeetingCollectionTile(PersistentCoverTile):

    implements(IMeetingCollectionTile)

    index = ViewPageTemplateFile('meeting_collection.pt')

    is_configurable = True
    is_editable = True
    short_name = _(u'msg_short_name_meeting_collection', default=u'Meeting Collection')
    configured_fields = []
    ...
    def populate_with_object(self, obj):
        super(MeetingCollectionTile, self).populate_with_object(obj)  # check permission
